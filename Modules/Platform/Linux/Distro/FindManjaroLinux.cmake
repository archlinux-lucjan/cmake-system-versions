...
if(CAT AND SED)
    if(DEFINED MANJARO_LINUX)
        if(EXISTS /etc/os-release)
            if(${OS_RELEASE_DISTRO} MATCHES "Manjaro Linux")
                set(MANJARO_LINUX 1)
            else()
                set(MANJARO_LINUX 0)
            endif()
        elseif(EXISTS /etc/manjaro-release)
            exec_program(${CAT} ARGS /etc/manjaro-release OUTPUT_VARIABLE DISTRO_RELEASE_TEXT)
            exec_program(${SED} ARGS ${DISTRO_RELEASE_REGEX} ${DISTRO_RELEASE_TEXT} OUTPUT_VARIABLE DISTRO_RELEASE_DISTRO)
            if(${DISTRO_RELEASE_DISTRO} MATCHES "Manjaro Linux")
                set(MANJARO_LINUX 1)
            else()
                set(MANJARO_LINUX 0)
            endif()
        else()
            if(${LSB_RELEASE_DISTRO} MATCHES "ManjaroLinux")
                set(MANJARO_LINUX 1)
            else()
                set(MANJARO_LINUX 0)
            endif()
