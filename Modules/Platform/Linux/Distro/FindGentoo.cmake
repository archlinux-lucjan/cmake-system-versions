...
if(CAT AND SED)
    if(DEFINED GENTOO)
        if(EXISTS /etc/os-release)
            if(${OS_RELEASE_DISTRO} MATCHES "Gentoo")
                set(GENTOO 1)
            else()
                set(GENTOO 0)
            endif()
        elseif(EXISTS /etc/gentoo-release)
            exec_program(${CAT} ARGS /etc/gentoo-release OUTPUT_VARIABLE DISTRO_RELEASE_TEXT)
            exec_program(${SED} ARGS ${DISTRO_RELEASE_REGEX} ${DISTRO_RELEASE_TEXT} OUTPUT_VARIABLE DISTRO_RELEASE_DISTRO)
            string(FIND ${DISTRO_RELEASE_DISTRO} "Gentoo" GENTOO_POSITION)
            if(${GENTOO_POSITION} GREATER -1)
                set(GENTOO 1)
            else()
                set(GENTOO 0)
            endif()
        else()
            if(${LSB_RELEASE_DISTRO} MATCHES "Gentoo")
                set(GENTOO 1)
            else()
                set(GENTOO 0)
            endif()
